                    ...
                    const int im2col_step)
{
    ...
    const int channels_out = input.size(1);//weight.size(0);
    const int channels_kernel = input.size(1);//weight.size(1);
    const int kernel_d_ = 1;//weight.size(2);
    const int kernel_h_ = 1;//weight.size(3);
    const int kernel_w_ = 1;//weight.size(4);
    ...
    for (int n = 0; n < batch/im2col_step_; ++n)
    {
        ...
        //    //printf("columns_gm: %d %d \n",columns_gm.size(0),columns_gm.size(1));
                                             ...
                                             const int im2col_step)
{
    ...
    for (int n = 0; n < batch/im2col_step_; ++n)
    {
        ...
        for (int g = 0; g < group; ++g)
        {
            ...
            columns_g.select(0, g) = grad_output_gm;
        }
    ...
    return {
        grad_input, grad_offset, grad_bias //grad_weight, grad_bias
    };
}
