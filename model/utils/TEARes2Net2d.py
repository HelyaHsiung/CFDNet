...
class Res2BlockShift(nn.Module):
    expansion = 1

    def __init__(self, inplanes, planes, stride=1, downsample=None, baseWidth=26, scale=4, stype='normal', n_segment=8):
        super(Res2BlockShift, self).__init__()
        width = int(math.floor(planes * (baseWidth / 64.0)))

        self.conv1 = nn.Conv2d(inplanes, width * scale, kernel_size=3, stride=stride, padding=1, bias=False)
        ...
        for i in range(self.nums):
            ...
            shifts.append(ShiftModule(width, n_segment=n_segment, n_div=2, mode='fixed'))

        shifts.append(ShiftModule(width, n_segment=n_segment, n_div=2, mode='shift'))
        self.convs = nn.ModuleList(convs)
        self.bns = nn.ModuleList(bns)
        self.shifts = nn.ModuleList(shifts)

        self.conv2 = nn.Conv2d(width * scale, planes, kernel_size=3, padding=1, bias=False)
...
class TEARes2Net(nn.Module):
    def __init__(self, block, layers, in_ch, baseWidth=26, scale=4, num_classes=1000):
        self.inplanes = 64
        super().__init__()
...
def res2net50(pretrained=False, **kwargs):
    ...
    """
    model = TEARes2Net(Bottle2neck, [3, 4, 6, 3], baseWidth=26, scale=4, **kwargs)
    if pretrained:
...
def tea50_8f(pretrained=False, **kwargs):
    ...
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = TEARes2Net(Bottle2neckShift, [3, 4, 6, 3], baseWidth=26, scale=4, **kwargs)
    if pretrained:
...
def res2net101_26w_4s(pretrained=False, **kwargs):
    ...
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = TEARes2Net(Bottle2neck, [3, 4, 23, 3], baseWidth=26, scale=4, **kwargs)
    if pretrained:
...
def res2net50_26w_6s(pretrained=False, **kwargs):
    ...
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = TEARes2Net(Bottle2neck, [3, 4, 6, 3], baseWidth=26, scale=6, **kwargs)
    if pretrained:
...
def res2net50_26w_8s(pretrained=False, **kwargs):
    ...
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = TEARes2Net(Bottle2neck, [3, 4, 6, 3], baseWidth=26, scale=8, **kwargs)
    if pretrained:
...
def res2net50_48w_2s(pretrained=False, **kwargs):
    ...
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = TEARes2Net(Bottle2neck, [3, 4, 6, 3], baseWidth=48, scale=2, **kwargs)
    if pretrained:
...
def res2net50_14w_8s(pretrained=False, **kwargs):
    ...
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = TEARes2Net(Bottle2neck, [3, 4, 6, 3], baseWidth=14, scale=8, **kwargs)
    if pretrained:
